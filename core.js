...
const UIIC = (() => {
  ...
  /* ---------- swap live form controls for plain text before capture ----------
     html2canvas does not reliably reproduce how a browser paints the text
     inside a live <input>/<textarea> (vertical centering, font metrics,
     checkbox chrome all vary by device/OS) which is why entered values could
     look shifted onto the underline. Plain text nodes are html2canvas's best
     supported case, so for the capture only, replace each control with a
     matching text element, then restore the live controls afterwards. */
  function prepareInputsForCapture(root) {
    const restores = [];

    root.querySelectorAll('.input-line input').forEach(input => {
      const cs = getComputedStyle(input);
      const stand = document.createElement('div');
      stand.textContent = input.value || '';
      stand.style.cssText =
        'font-family:' + cs.fontFamily + ';font-size:' + cs.fontSize + ';' +
        'color:' + cs.color + ';line-height:normal;padding:' + cs.padding + ';' +
        'min-height:1em;white-space:pre-wrap;word-break:break-word;';
      input.style.display = 'none';
      input.insertAdjacentElement('afterend', stand);
      restores.push(() => { stand.remove(); input.style.display = ''; });
    });

    root.querySelectorAll('.field textarea').forEach(ta => {
      const cs = getComputedStyle(ta);
      const stand = document.createElement('div');
      stand.textContent = ta.value || '';
      stand.style.cssText =
        'font-family:' + cs.fontFamily + ';font-size:' + cs.fontSize + ';' +
        'color:' + cs.color + ';line-height:' + cs.lineHeight + ';padding:' + cs.padding + ';' +
        'border:' + cs.border + ';border-radius:' + cs.borderRadius + ';' +
        'min-height:' + cs.height + ';white-space:pre-wrap;word-break:break-word;background:#fff;';
      ta.style.display = 'none';
      ta.insertAdjacentElement('afterend', stand);
      restores.push(() => { stand.remove(); ta.style.display = ''; });
    });
    ...
    root.querySelectorAll('input[type=checkbox]').forEach(cb => {
      ...
      mark.style.cssText =
        ...
        'width:13px;height:13px;border:1px solid #555;border-radius:2px;' +
        'font-size:11px;line-height:1;color:#173a63;font-weight:700;background:#fff;vertical-align:middle;';
      cb.style.display = 'none';
      cb.insertAdjacentElement('afterend', mark);
      restores.push(() => { mark.remove(); cb.style.display = ''; });
    });
  ...
  async function exportToPDF(containerId, filename, statusElId) {
    ...
    try {
      ...
      restore = prepareInputsForCapture(src);

      // Force a fixed "desktop" viewport for the capture so the layout
      // (which fields sit side-by-side vs. stack) is identical on every
      // device, instead of following whatever width/orientation the
